...
/*   Updated: 2021/12/20 13:39:26 by msanjuan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	retrieveLineNumber_bonus(char *path)
{
	int		fd;
    char    *line;
	int line_count;
...
void	createMap_bonus(char *path, t_data *data)
{
	char *line;
	int row;
	int i;
	...
	data->map.line_count = retrieveLineNumber_bonus(path);
	data->map.path = path;
	...
	else
	{
		while ((line = get_next_line(data->map.fd)) != NULL)
		{
			...
			if (!data->map.map[row])
				return (ft_free_bonus(data->map.map));
			while (line[i] != '\0')
